...
export default function PDFViewer({ pdfUrl, highlightedChunks, currentPage, onPageChange }: Props) {
  ...
  if (isImage) {
    return <ImageWithBoxes url={pdfUrl!} highlightedChunks={highlightedChunks} />;
  }
...
// Image overlay: same verification palette as the PDF path. The bbox the
// backend emits for image uploads is in original-image pixel coords (chunker
// uses sx=sy=1 for raw images). We render the image at its natural size
// inside a relatively-positioned wrapper and draw a canvas overlay at the
// same dimensions so coordinates align without any extra scale math.
function ImageWithBoxes({ url, highlightedChunks }: {
  url: string;
  highlightedChunks: ChunkResult[];
}) {
  ...
  // Resize observer: recompute scale when the rendered image size changes
  // (user expands/collapses panes, window resizes, etc.).
  useEffect(() => {
    ...
    const update = () => {
      ...
      setDims({ w: el.naturalWidth, h: el.naturalHeight,
                scale: el.clientWidth / el.naturalWidth });
    };
    update();
  ...
  useEffect(() => {
    ...
    // image uploads are single-page — match anything on page 0 OR no page set
    const page = highlightedChunks.filter(
      (c) => (c.bbox.page_num ?? 0) === 0,
    );

    for (const chunk of page) {
      const { x0, y0, x1, y1 } = chunk.bbox;
      if (x1 - x0 <= 0 || y1 - y0 <= 0) continue;
      const sx = x0 * dims.scale;
      const sy = y0 * dims.scale;
      const sw = (x1 - x0) * dims.scale;
      const sh = (y1 - y0) * dims.scale;
      ...
      <div className="flex flex-1 items-center justify-center overflow-auto bg-background p-4">
        <div className="relative inline-block">
          ...
          <img
            ...
            className="max-h-full max-w-full rounded-sm object-contain shadow-2xl shadow-black/40 block"
          />
          <canvas
            ref={overlayRef}
            className="absolute inset-0 pointer-events-none"
